...
namespace NHibernate.Test.NHSpecificTest.GH2563
{
	...
	public class ByCodeFixtureAsync : TestCaseMappingByCode
	{
		...
		public async Task SerializationOfConnectedEntitiesWithLazyPropertiesAsync()
		{
			using (var session = OpenSession())
			{
				var result = await (session.Query<Entity>().Where(e => e.Id < 3).ToListAsync());
		...
		[Test]
		public async Task SerializationOfCollectionConnectedEntitiesWithLazyPropertiesAsync()
		{
			using (var session = OpenSession())
			{
				var result = await (session.Query<Entity>().Where(e => e.Id > 2 && e.Id < 5).ToListAsync());
		...
		[Test]
		public async Task SerializationOfManyToManyConnectedEntitiesWithLazyPropertiesAsync()
		{
			using (var session = OpenSession())
			{
				var result = await (session.Query<Entity>().Where(e => e.Id > 4 && e.Id < 7).ToListAsync());
		...
		[Test]
		public async Task SerializationOfCrossManyToManyConnectedEntitiesWithLazyPropertiesAsync()
		{
			using (var session = OpenSession())
			{
				var result = await (session.Query<Entity>().Where(e => e.Id > 6).ToListAsync());
